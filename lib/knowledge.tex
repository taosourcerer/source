...
\scalebox{0.9}{
	\begin{tikzpicture}
  		...
  		\node (nopatterns) at (3.5,1) {no patterns};
  		\node (patterns) at (6,0) {patterns};
		\node[fill=orange!20] (one) at (8,0.6) {one model};
		\node[fill=orange!20] (all) at (8,-0.6) {all models};
  		\node (mealy) at (3,-3) {Mealy};
		\node (pushdown) at (4,-3) {pushdown};
		\node (lba) at (5,-3) {LBA};
  		\node (turing) at (6,-3) {Turing};
  		\node (uncountable) at (7,-3) {uncountable};
  		\node (unknown) at (8,-3) {unknown};
		\node (speed) at (5,-4.25) {speed};
		\node (length) at (6,-4.25) {length};
		\node (memory) at (7,-4.25) {memory};
		\node (random) at (1.75, -0.5) {random};
		\node (incompressable) at (3, -0.5) {incompressable};
...
It does not matter which member we choose\footnote{Be it for example C or Phython}, because for all languages $A$ and $B$ and all strings $x$ there exists a constatnt $c$ such that $K_A(x) \leq K_B(x) + c$, as explained in the \texttt{1.2.4} subsection.
